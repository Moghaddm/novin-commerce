...
internal class ProductManagementDataSeedContributor : IDataSeedContributor, ITransientDependency
{
    ...
    public async Task SeedAsync(DataSeedContext context)
    {
        ...
        /* Products */
        var shoeProduct = await _productManagerService.CreateAsync(new Product("Walking Shoe 5x0x",
            "Good Shoe For Walk or Run", 350000, 15, ProductStockState.InStock, shoeCategory, kooroshCompany));
        var tShirtProduct = await _productManagerService.CreateAsync(new Product("T-Shirt 2y31",
            "Good T-Shirt For Covering in Public", 500000, 23, ProductStockState.InStock, shirtCategory, refahCompany));
        var hatProduct = await _productManagerService.CreateAsync(new Product("Sun Hat 4q12", "Good Hat For Sunny Days",
            95000, 4, ProductStockState.InStock, shirtCategory, refahCompany));
